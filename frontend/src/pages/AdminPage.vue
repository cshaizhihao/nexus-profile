      ...
      <div v-else-if="activeTab === 'categories'" class="grid gap-6">
        ...
        <div class="grid gap-3">
          <article v-for="category in store.categories" :key="category.id" class="flex items-center justify-between rounded-2xl border border-slate-200 bg-white/70 p-4">
            ...
            <div class="flex gap-2"><button class="ghost-btn" @click="editCategory(category)">编辑</button><button class="danger-btn" @click="removeCategory(category.id)">删除</button></div>
          </article>
        </div>
      </div>
        ...
        <div class="grid gap-3">
          <article v-for="link in store.links" :key="link.id" class="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white/70 p-4">
            <div class="flex items-center gap-3">
              <img v-if="link.iconUrl" :src="link.iconUrl" class="h-10 w-10 rounded-xl object-cover" />
              <div><h3 class="font-semibold">{{ link.title }}</h3><p class="break-all text-sm text-slate-400">{{ link.url }}</p></div>
            </div>
            <div class="flex gap-2"><button class="ghost-btn" @click="editLink(link)">编辑</button><button class="danger-btn" @click="removeLink(link.id)">删除</button></div>
          </article>
        </div>
      </div>
    </div>
...
import { api, type NavCategory, type NavLink } from '../api/client'
...
watch(() => store.siteConfig, (config) => {
  if (!config) return
  Object.assign(siteForm, { title: config.title, subtitle: config.subtitle, description: config.description, avatarUrl: config.avatarUrl || '', backgroundType: config.backgroundType, backgroundValue: config.backgroundValue, customCss: config.customCss })
}, { immediate: true })
...
async function uploadSiteAvatar(event: Event) { const file = (event.target as HTMLInputElement).files?.[0]; const path = await upload(file); if (path) { siteForm.avatarUrl = path; flash('头像已上传') } }
async function uploadLinkIcon(event: Event) { const file = (event.target as HTMLInputElement).files?.[0]; const path = await upload(file); if (path) { linkForm.iconUrl = path; flash('图标已上传') } }
