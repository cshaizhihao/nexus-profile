<template>
  <section class="grid gap-7">
    ...
    <div v-if="store.projects.length" class="grid gap-5 md:grid-cols-6">
      <a v-for="(item, index) in store.projects" :key="item.id" :href="item.url || '#'" target="_blank" rel="noreferrer" class="archive-card depth-card group overflow-hidden p-5 transition hover:-translate-y-1 hover:shadow-[0_40px_100px_rgba(65,47,25,.18)]" :class="index === 0 ? 'md:col-span-4 md:row-span-2' : 'md:col-span-2'">
